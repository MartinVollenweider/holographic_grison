...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public static class RealWorldTerrainTreesGenerator
    {
        ...
        public static string url
        {
            get
            {
                string format = string.Format(RealWorldTerrainCultureInfo.numberFormat, "node({0},{1},{2},{3})['natural'~'tree'];(._;>;);out;node({0},{1},{2},{3});(way(bn)['natural'~'wood|tree_row'];way(bn)['landuse'~'forest|park'];);(._;>;);out;(way['landuse'~'forest|park']({0},{1},{2},{3});relation['landuse'~'forest|park']({0},{1},{2},{3}););out;>;out skel qt;", prefs.coordinatesTo.y, prefs.coordinatesFrom.x, prefs.coordinatesFrom.y, prefs.coordinatesTo.x);
